...
# ── Safe imports — won't crash even if supporting files are old ─────────────
try:
    ...
    print(f"[GeoTrace] DB init failed: {e}")
    DB_OK = False
    def save_result(r): pass
...
try:
    ...
    print(f"[GeoTrace] threat_logic import failed: {e}")
    THREAT_OK = False
...
def safe_classify(domain, ip, geo):
    if not THREAT_OK:
        return "UNKNOWN", 0, ["Threat module unavailable."]
    try:
        result = classify_threat(domain, ip, geo)
        # Handle old versions that return only 1 value (just a string)
...
def analyze():
    ...
    threat_level, threat_score, threat_reasons = safe_classify(domain, ip, geo)
